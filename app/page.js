...
export default function Home() {
  return (
    <main className="block container justify-center items-center w-full mx-auto p-4">
      <div className="container text-end px-10 py-5 ">
        <ThemeSwitch />
      </div>
      ...
      <div className="container  lg:columns-5 md:columns-3 sm:columns-1 px-10 py-5">
        {
          sportsData.map((card, index) => {
            ...
            return (
              <Card key={index} img_link={card.img_link} cardheader={<CardHeader title={card.title} events={card.num_events} sportType={card.sport_type} />} />
            )
      ...
      <div className="container w-9/12 mx-auto  lg:columns-3 md:columns-2 sm:columns-1 px-15 py-5">
...

        {
          eventData.map((card, index) => {
            ...
            return (
              <Card key={index} img_link={card.img_link} cardheader={<CardHeader2 title={card.title} month={card.month} day={card.day_name} time={card.time} content={card.content} />} />
            )
